...
流程图和定性对比表沿用图片生成与参考图输入能力，不需要另装绘图库。复杂的连接关系先用文字或表格核对，生成后检查箭头与行列。

真实数字的图表需要原始数据、单位、时间范围和来源，以及可运行确定性绘图工具的环境。例如 Python 加 Matplotlib，或支持 Vega-Lite 的环境；按实际选择安装依赖，并保留绘图源文件。仓库没有附带这些绘图库或通用数据绘图脚本。图片模型可以生成单独的贝拉插画作为配图，不能凭外观猜数值、重绘坐标或更改比例。

[查看两张 Demo 与提示词](docs/diagram-demos.md) · [图示规则](references/diagrams-and-charts.md)
